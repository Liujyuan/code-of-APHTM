...
def align_loss(x, y, alpha=2):
    return (x - y+1e-6).norm(p=2, dim=1).pow(alpha).mean()+1e-6


def uniform_loss(x, t=2):
   # print('x',x.shape)
    return torch.pdist(x+1e-6, p=2).pow(2).mul(-t).exp().mean().log()+1e-6
def au_loss(x,y):
    a_loss = align_loss(x, y, alpha=2)
    u_loss = (uniform_loss(x) + uniform_loss(y)) / 2
   # print('a_loss+u_loss',a_loss,u_loss)
    return a_loss+u_loss
